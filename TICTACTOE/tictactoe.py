...
class TicTacToe:
    ...
    # Convert board state to an array for the neural network (matrix to vector)
    ...
    def nn_move(self):
        ...
        for col in range(3):
            for row in range(3):
                if self.board[row][col] == '.': #check if empty 
                    s = copy.deepcopy(self.board) #copy board 
                    s[row][col] = 'X' # do a move
                    arr = self.brd2arr(s) # convert from matrix to vector (for the NN)
    ...
    def play_random_game(self):
        ...
        while True:
            if self.current_player == 'X':
                ...
                row, col = random.randint(0, 2), random.randint(0, 2) # get empty place - with no brain
            if self.make_move(row, col): #and check if win
                self.print_board()
                break
            elif self.is_draw():
                print("It's a draw!")
